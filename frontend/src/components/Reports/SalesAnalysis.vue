...
const stats = ref([
  { label: '销售总额', value: '¥0', icon: 'Money', color: '#67c23a' },
  { label: '销售单数', value: '0', icon: 'List', color: '#409eff' },
  { label: '平均单价', value: '¥0', icon: 'TrendCharts', color: '#e6a23c' },
  { label: '客户数', value: '0', icon: 'User', color: '#f56c6c' }
])
...
async function loadData() {
  ...
  try {
    ...
    const res = await reportApi.getSalesSummary(params)
    ...
    stats.value[3].value = new Set(data.map((d: any) => d.customer_id)).size.toString()
