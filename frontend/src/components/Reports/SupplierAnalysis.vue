<template>
  <div class="supplier-analysis">
    ...
    <el-card style="margin-top: 20px" v-loading="loading">
      <template #header>供应商采购金额分布</template>
      <div ref="chartRef" style="height: 400px"></div>
    </el-card>
    
    <el-card style="margin-top: 20px">
      <template #header>供应商排行榜</template>
      <el-table :data="tableData" stripe>
        <el-table-column type="index" label="排名" width="80" />
        <el-table-column prop="name" label="供应商名称" />
        <el-table-column prop="code" label="供应商编码" width="150" />
        <el-table-column prop="order_count" label="采购单数" align="right" width="120" />
        <el-table-column prop="total_amount" label="采购金额" align="right" width="150">
...
const stats = ref([
  { label: '供应商总数', value: '0', icon: 'Shop', color: '#409eff' },
  { label: '活跃供应商', value: '0', icon: 'CircleCheck', color: '#67c23a' },
  { label: '采购总额', value: '¥0', icon: 'Money', color: '#e6a23c' }
])

async function loadData() {
  loading.value = true
  try {
    const res = await reportApi.getSupplierPerformance()
    const data = res.data || []
